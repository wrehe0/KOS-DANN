...
3. **Фиксация**, сохранение - `git commit -m 'Update README.md - fix'` - сохранение с комментарием
...
> где вместо **Your name** - ваш username
...
> Если вы пользуетесь git на чужом компьтере, то используйте `--local`
...
```git diff```
сравнение до ```git add и git commit -m```. Чтобы выйти из режима просмотра изменений, выполните `Q`

```git diff --staged```
Чтобы выйти из режима просмотра изменений, выполните `Q`
